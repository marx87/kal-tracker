...
- (void)handleMethodCall:(FlutterMethodCall *)call
  ...
  if ([@"getDeviceInfo" isEqualToString:call.method]) {
    ...
    if (@available(iOS 26.1, *)) {
      // Xcode 16.4 does not declare this iOS 26.1 selector yet. Resolve it at
      // runtime so the same binary still reports the value on newer systems
      // without asking the older SDK to type-check an unknown API.
      SEL selector = NSSelectorFromString(@"isiOSAppOnVision");
      if ([info respondsToSelector:selector]) {
        IMP implementation = [info methodForSelector:selector];
        BOOL (*getter)(id, SEL) = (BOOL (*)(id, SEL))implementation;
        isiOSAppOnVision = [NSNumber numberWithBool:getter(info, selector)];
      }
    }
    NSError *error = nil;
